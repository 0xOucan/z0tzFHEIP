...
## Confidential DeFi as a composition target

V6.5 closes the holder leak; it does not, by itself, give the user anywhere private to *put* their balance to work. That gap closes by composition rather than by extending V6.5. The natural place is a confidential vault — a contract that accepts encrypted deposits, runs an underlying strategy on the unwrapped pool, and tracks per-user shares as ciphertext. We did not build that primitive. Tezcatli did. So the question becomes whether the V6.5 stack composes with Tezcatli's vaults the same way it composed with Circle's bridge.

It does. The user's smart account never appears in the vault any more than it appears in CCTP. A fresh DeFi stealth derives from `(passkey, originChainId, vaultChainId, vaultAddress, index)`, the ledger debits to it, the stealth deposits, and the stealth dies. The vault sees one ephemeral depositor per deposit; it never learns the user's wallet address, and never sees two of the user's deposits as related. On withdraw the wallet knows where the funds came from — the salt encodes the origin chain — and routes them home automatically: ledger A → vault on B → ephemeral on B → CCTP burn → ephemeral on A → ledger A. The composition pattern is the one V6.5 already uses for cross-chain cash-out; the only addition is that the &ldquo;protocol&rdquo; in the middle is a vault rather than a bridge.

A coordinator-driven strategy keeps the picture honest. Idle USDC sitting in a wrapper earns nothing, so after every deposit the relayer signs `coordinatorDeployToStrategy` to push the idle balance into Aave V3, and before every withdraw it signs `coordinatorRedeemFromStrategy` to pull aTokens back so the wrapper has plaintext liquidity for the unshield. The stealth has no role in the strategy — it's a one-shot depositor — and the relayer is the only party that persists across deposits, so it's the right party to maintain the vault's invariants. None of this changes who the chain sees: the deposit and withdraw events still name a stealth, and the strategy events name the relayer's coordinator role, never the user.

What's worth saying out loud is what this proves about the composition bet. The bet was that stealth-as-proxy generalizes to *any* permissionless protocol without modifying the protocol. CCTP was the first concrete proof; Tezcatli is the second, and the harder one in some ways because a vault has internal accounting that CCTP doesn't — shares, principal, fees, snapshots. The V6.5 wallet reads those numbers from chain on every render (no local cost-basis cache, no per-position state map), so the privacy story doesn't depend on any client-side bookkeeping that could leak through a logfile or a dev-tools window.

## Compliance at the integration boundary

The other thing that lands with the Tezcatli composition is the part of the privacy stack the V6.5 essay deliberately did not address: compliance. A wallet that hides amounts and identities cannot also be the place that decides whether a sanctioned address gets to use the system. That decision belongs at the boundary, not in the wallet, and it has to be a public refusal — not a quiet block, not a custody seizure, not an auto-return. The user has to be able to see why a payment didn't go through and walk away with their keys.

`Z0tzComplianceGate` is that refusal contract. It exposes `canShield(token, depositor, amount)` and `canUnshield(token, beneficiary, amount)`; both return a yes/no with a typed reason code per FHEIP-0010. The gate has zero token-moving authority — it answers questions, it doesn't move value. The wallet pre-flights every shielded movement against it via `eth_call` before paying any gas, so a denied operation costs nothing and surfaces in the GUI as plain English ("KYC required", "OFAC block-list", "daily cap exceeded") instead of a hex selector. The gate is composed of three pieces: a KYC registry that stores yes/no plus an optional expiry per address (no documents, no PII), a sanctions list consulted before the gate's own deny-list, and an append-only depositor registry that lets `canUnshield` make sure a clean-looking withdraw isn't actually exiting a position that was opened by a sanctioned counterparty.

Default posture: AML deny-list and OFAC list on at the contract layer, geofencing on at the relayer HTTP layer, KYC off until an integrator opts in. Z0tz the wallet does not demand KYC from end users; an SDK integrator (a dApp, an institution) flips it on for their own users when their flow needs it. The gate's `enabled` flag defaults to false during bring-up so the wiring can be tested without rejecting anyone, and is admin-toggled to true at production deploy. Two-step admin transfers throughout (Ownable2Step style) so a compromised admin key isn't a one-step disaster.

The deeper move here is the same composition move V6.5 makes everywhere else: the gate is a separate contract from the wallet, the relayer's geofence is separate from both, and the KYC supplier is separate from all three. Each piece can be replaced or audited on its own. The wallet stays a pure non-custodial primitive. The gate stays a pure predicate. The relayer stays a thin HTTP service. None of the three holds anyone's funds; the user's keys are still the only thing that moves value, regardless of what any one of these layers decides about a given operation.
